...
class TileEntityPressureCrusher extends TileEntityEnvironment with AddressPastable with ItemStorage with IInventory {
  ...
  node = node_
  val recipes = Map("oreIron" -> AwesomeSauceComponents.dustIron, "oreGold" -> AwesomeSauceComponents.dustGold, "oreAluminum" -> AwesomeSauceComponents.dustAluminum, "oreTin" -> AwesomeSauceComponents.dustTin, "oreCopper" -> AwesomeSauceComponents.dustCopper, "oreSilver" -> AwesomeSauceComponents.dustSilver)
  ...
  def crush(context: Context, arguments: Arguments): Array[AnyRef] = {
    ...
    if (node_.tryChangeBuffer(-100)) {
      if (crushedSlot == null)
        crushedSlot = new ItemStack(recipes(ore), 2)
      else if (crushedSlot.getItem() == recipes(ore))
  ...
  def decrStackSize(i: Int, amount: Int): net.minecraft.item.ItemStack = if (i == 0) { return crushedSlot.splitStack(amount) } else return crushingSlot.splitStack(amount)
